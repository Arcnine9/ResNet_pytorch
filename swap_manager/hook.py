...
class HookManager:
    def __init__(self, swap_manager: SwapManager, event_file: str, model: nn.Module, verbose: bool = None):
        ...
        # ★ 激进优化：批量处理状态
        self._pending_d2h_tensors: List[int] = []  # 延迟同步的 D2H tensor
        self._pending_h2d_tensors: List[int] = []  # 预取的 H2D tensor
    ...
    def probe_modules(self, sample_input: torch.Tensor, sample_target: Optional[torch.Tensor] = None, criterion: Optional[nn.Module] = None):
        ...
        def make_forward_probe(name):
            def hook_fn(m, inp, out):
                ...
                time_to_module[current_time] = (name, True)
                if self._local_verbose:
        ...
        def make_backward_probe(name):
            def hook_fn(m, grad_in, grad_out):
                with self._probe_lock:
                    self._probe_time += 1
                    current_time = self._probe_time
                time_to_module[current_time] = (name, False)
                if self._local_verbose:
        ...
        def forward_hook(module, inputs, output):
            ...
            for ev in events:
                ...
                if ev.event_type == "extract":
                    ...
                    # ★ 激进优化：默认不等待，加入延迟队列
                    # 实际同步延迟到下一层或内存紧张时
                    wait_d2h_batch.append(tid)
                    if verbose:
                        log_lifecycle("wait_d2h", tid, module_name, ev.issued_time)

                elif ev.event_type == "h2d":
                    # 前向通常不需要 H2D，但保留接口
                    swap_mgr.launch_h2d(tid, compute_stream, flag=True)
                    if verbose:
                        log_lifecycle("h2d", tid, module_name, ev.issued_time)

                elif ev.event_type == "wait_h2d":
                    swap_mgr.wait_h2d_finished(tid, compute_stream, need_wait=True)
                    if verbose:
                        log_lifecycle("wait_h2d", tid, module_name, ev.issued_time)

            # ★ 激进优化：批量处理 wait_d2h，默认不阻塞
            if wait_d2h_batch:
                # 默认不等待，让 D2H 在后台继续
                # 如果需要强制同步（如内存紧张），可以改为 need_wait=True
                swap_mgr.sync_all_d2h(wait_d2h_batch, compute_stream, need_wait=False)
        ...
        def backward_hook(module, grad_input, grad_output):
            ...
            for ev in events:
                ...
                if ev.event_type == "d2h":
                    ...
                    # 反向时通常需要确保 D2H 完成（如要释放内存）
                    swap_mgr.wait_d2h_finished(tid, compute_stream, need_wait=True)
                    if verbose:
                        log_lifecycle("wait_d2h", tid, module_name, ev.issued_time)

                elif ev.event_type == "h2d":
                    # ★ 激进优化：收集 H2D 请求，批量预取
                    h2d_batch.append(tid)
                    if verbose:
                        log_lifecycle("h2d", tid, module_name, ev.issued_time)

                elif ev.event_type == "wait_h2d":
                    # ★ 激进优化：收集等待请求，批量智能等待
                    wait_h2d_batch.append(tid)
                    if verbose:
                        log_lifecycle("wait_h2d", tid, module_name, ev.issued_time)

            # ★ 激进优化：批量启动 H2D 预取
            if h2d_batch:
                swap_mgr.h2d_prefetch(h2d_batch, compute_stream)

            # ★ 激进优化：批量智能等待 H2D
            # 先查询是否已完成，避免不必要的阻塞
            if wait_h2d_batch:
                for tid in wait_h2d_batch:
                    # 先检查是否已完成（预取足够提前）
                    if not swap_mgr.is_h2d_finished(tid):
                        # 未完成才等待
                        swap_mgr.wait_h2d_finished(tid, compute_stream, need_wait=True)
    ...
    def reset(self):
        """重置状态"""
        self._pending_d2h_tensors.clear()
        self._pending_h2d_tensors.clear()
